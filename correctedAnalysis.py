...
import pandas as pd
df = pd.read_json('beerabv.json').astype(str)

df.head
df.info()

beer_dim = df[['name', 'tagline','description']].reset_index(drop=True).astype(str)
...
food_pairing_dim = df[['food_pairing']].reset_index(drop=True).astype(str)
...
brewer_tips_dim = df[['brewers_tips']].astype(str)
